...
ObjType = ["Not_foraging"]  # Default fish family
...
def add_one(event=None):
    """Adds a new annotation with no foraging and (0,0) location."""
    global annotations, ObjType, ObjID, Damselfish_present, current_frame_index, root
    if root.focus_get() and isinstance(root.focus_get(), (ctk.CTkEntry, Entry)):
        return
    ObjID[0] = fish_name.get()
...
def add_two(event=None):
    """Adds a new annotation with foraging and (0,0) location."""
    global annotations, ObjType, ObjID, Damselfish_present, current_frame_index, root
    if root.focus_get() and isinstance(root.focus_get(), (ctk.CTkEntry, Entry)):
        return
    ObjID[0] = fish_name.get()
...
def add_three(event=None):
    """Adds a new annotation with aggressive interaction with tang and (0,0) location."""
    global annotations, ObjType, ObjID, Damselfish_present, current_frame_index, root
    if root.focus_get() and isinstance(root.focus_get(), (ctk.CTkEntry, Entry)):
        return
    ObjID[0] = fish_name.get()
    annotation = {
        "Frame": current_frame_index[0],
        "ClickType": "Aggressive_with_tang",
        "ObjID": ObjID[0]
    }
    annotations.append(annotation)
    print(f"Annotation added: {annotation}")
...
# Toggle Fish Family
def toggle_obj_type():
    if ObjType[0] == "Not_foraging":
        ObjType[0] = "Foraging"
        button_toggle_obj_type.configure(text="Foraging")
    elif ObjType[0] == "Foraging":
        ObjType[0] = "Aggressive_with_tang"
        button_toggle_obj_type.configure(text="Aggressive_with_tang")
    elif ObjType[0] == "Aggressive_with_tang":
        ObjType[0] = "NA"
        button_toggle_obj_type.configure(text="NA")
    elif ObjType[0] == "NA":
        ObjType[0] = "Not_foraging"
        button_toggle_obj_type.configure(text="Not_foraging")


#Toggle damselfish presence
def toggle_damselfish_present():
    if Damselfish_present[0] == "no damselfish present":
        Damselfish_present[0] = "damselfish present"
        button_toggle_damselfish_present.configure(text="damselfish present")
    elif ObjType[0] == "damselfish present":
        ObjType[0] = "no damselfish present"
        button_toggle_damselfish_present.configure(text="no damselfish present")
...
def next_special_frame():
    ...
    next_idx = current_frame_index[0] + 1

    while next_idx < total_frames:
        if (next_idx - special_frame_start) % special_frame_interval == 0:
            break
        next_idx += 1

    current_frame_index[0] = next_idx
    display_frame_index(next_idx)
...
button_set_special_frame.pack_forget()
...
button_toggle_click.pack_forget()
...
button_add_exit = ctk.CTkButton(frame_controls, text="Not_foraging ('1')", command=add_one, height = 20)
button_add_exit.pack(pady=5)

button_add_exit = ctk.CTkButton(frame_controls, text="Foraging ('2')", command=add_two, height = 20)
button_add_exit.pack(pady=5)

button_add_exit = ctk.CTkButton(frame_controls, text="Aggressive_with_tang ('3')", command=add_three, height = 20)
button_add_exit.pack(pady=5)

button_add_exit = ctk.CTkButton(frame_controls, text="NA ('4')", command=add_four, height = 20)
button_add_exit.pack(pady=5)
